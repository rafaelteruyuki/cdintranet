...
class Stg_Helper_Custom_Forms
{
    ...
    public static function getCustomFields()
    {
        ...
        $result = array(
            array('id' => 'ticket_fields', 'text' => __('Ticket fields', STG_HELPDESK_TEXT_DOMAIN_NAME),
                'children' => array(
                    array('text' => __('Category', STG_HELPDESK_TEXT_DOMAIN_NAME), 'id' => '[ticket_field \'category\']', 'label' => 'Category'),
                    array('text' => __('Tags', STG_HELPDESK_TEXT_DOMAIN_NAME), 'id' => '[ticket_field \'tags\']', 'label' => 'Tag'),
                    array('text' => __('Subject (required field)', STG_HELPDESK_TEXT_DOMAIN_NAME), 'id' => '[ticket_field \'subject\']', 'label' => 'Subject'),
                    array('text' => __('Message (required field)', STG_HELPDESK_TEXT_DOMAIN_NAME), 'id' => '[ticket_field \'message\']', 'label' => 'Description'),
                    array('text' => __('Files', STG_HELPDESK_TEXT_DOMAIN_NAME), 'id' => '[ticket_field \'files\']'),
                    array('text' => __('reCAPTCHA', STG_HELPDESK_TEXT_DOMAIN_NAME), 'id' => '[ticket_field \'reCAPTCHA\']'),
                )),
            array('id' => 'contact_fields', 'text' => __('Contact fields', STG_HELPDESK_TEXT_DOMAIN_NAME),
                'children' => array(
                    array('text' => __('Name (required field)', STG_HELPDESK_TEXT_DOMAIN_NAME), 'id' => '[contact_field \'name\']', 'label' => 'Name'),
                    array('text' => __('Email (required field)', STG_HELPDESK_TEXT_DOMAIN_NAME), 'id' => '[contact_field \'email\']', 'label' => 'Email'),
                )),
            ...
            foreach ($macro['children'] as $child) {
                ...
                switch ($child['id']) {
                    ...
                    case '[ticket_field \'files\']':
                        ...
                        break;
                    case '[ticket_field \'reCAPTCHA\']':
                        $tmp['replace'] = self::getStandartFieldReCAPTCHA();
                        break;
    ...
    public static function getStandartFieldCategory(){
        return \StgHelpdesk\Ticket\Stg_Helpdesk_TicketCategory::getSelectList(null, null, 'stgh_standart_field');
    }

    public static function getStandartFieldTag(){
        return stgh_ticket_tags_list(null, null, 'stgh_standart_field');
    }
    ...
    public static function getStandartFieldReCAPTCHA(){
        ...
        if($key)
            return '<div id="g-recaptcha" class="g-recaptcha" data-sitekey="'.$key.'"></div>';
    ...
    public static function dateConstructor($fieldMeta,$fieldName,$value = false, $required, $isAdmin){
        ...
        $format = isset($fieldMeta['custom_field_format_meta'])? $fieldMeta['custom_field_format_meta']:'';
            ...
            foreach($optionsArr as $option)
            {
                if(!empty($option)) {
                    ...
                    if($value !== false && is_array($value))
                        ...
                        $checked = '';
                    $options .= "<input {$disabled} {$checked} type='checkbox' name='stgh_custom_fields[{$fieldName}][]' {$required} value='{$currValue}' id='{$currValue}'/><label for='{$currValue}'>{$text}</label>";
                }
            }
        ...
        if(count($optionsArr)>1)
        {
            foreach($optionsArr as $option)
            {
                if(!empty($option)) {
                    ...
                    $checked = ($currValue == $value)?'checked="checked"':'';
                    $options .= "<input {$disabled} {$checked} type='radio' name='stgh_custom_fields[{$fieldName}]' {$required} value='{$currValue}' id='{$currValue}' /><label for='{$currValue}'>{$text}</label>";
                }
            }
        }
